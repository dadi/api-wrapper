...
module.exports = function(APIWrapper) {
  APIWrapper.prototype._getModelFunction = function(requestObject) {
    const model = this.options.apiInstance(this.collection)
  ...
  APIWrapper.prototype._processRequest = function(requestObject, core) {
    ...
    return passport(this.passportOptions, request).then(
        ...
        return authenticatedRequest(options)
          .then(response => {
            if (this.requiredFeatures) {
              const supportedFeatures =
                response.headers['x-dadi-supports'] || []
              const missingFeatures = this.requiredFeatures.filter(feature => {
                return supportedFeatures.indexOf(feature) === -1
              })

              if (missingFeatures.length > 0) {
                const error = new Error(
                  `API does not support features: ${missingFeatures.join(';')}`
                )
          ...
          .catch(err => {
            ...
            if (
              ...
              err.response.headers['www-authenticate'].indexOf(
                'invalid_token'
              ) !== -1
            ) {
              debug(
                'The request failed due to an invalid bearer token. Requesting a new one...'
              )
